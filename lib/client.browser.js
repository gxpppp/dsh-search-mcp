		...
		//#region controller (whole-section staged draft)
			var SearchMcpCardController = class {
				...
				async readSecretState() {
					...
					if (this.draft === null && (changed || !wasReady)) this.publish();
				}
			...
			actions() {
				return {
						...
						for (const row of draft.servers) {
							...
							if (row.apiKey.trim() !== "") {
								const ref = credentialRefFor(row);
									try {
										const response = await this.api.credentials.set({ ref, value: row.apiKey.trim() });
										landed = response.result.ok && landed;
									} catch {
										landed = false;
									}
					...
					for (const [field, value, previous] of writes) {
