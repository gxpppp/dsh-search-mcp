...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".smcp_headBtn{display:flex;align-items:flex-start;gap:8px;width:100%;text-align:left;background:0 0;border:none;padding:0;font:inherit;cursor:pointer}",
			".smcp_head{display:flex;flex-direction:column;gap:4px;flex:1;min-width:0}",
			...
			".smcp_chevron{color:var(--dsw-alias-label-tertiary);transition:transform .15s ease;flex-shrink:0;margin-top:2px}",
			".smcp_chevronOpen{transform:rotate(180deg)}",
			".smcp_body{display:flex;flex-direction:column;gap:0}",
